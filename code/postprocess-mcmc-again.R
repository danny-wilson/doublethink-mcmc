...
# Re-processing using updated burnin and thinning interval
#
###################

help = paste(
"Usage: Rscript postprocess-mcmc-again.R config_file burnin thinning",
sep="\n")
...
thinning = as.integer(args[3])
cat("Changing thinning interval from ",old.thinning," to ",thinning,"\n")
stopifnot(thinning>=1)

# Output files: names incorporate updated burning and thinning interval
SUFFIX = paste0(".burnin",burnin,".thinning",thinning)
results.posterior.inclusion.probs = paste0("results.posterior-inclusion-probs",SUFFIX,".tsv")
results.group.posteriors.inclusion.probs = paste0("results.group-posterior-inclusion-probs",SUFFIX,".tsv")
results.matrix.hmp.stats = paste0("results.matrix-hmp-stats",SUFFIX,".tsv")
results.top.log.posteriors = paste0("results.top-log-posteriors",SUFFIX,".tsv")
results.top.log.hmp = paste0("results.top-log-hmp",SUFFIX,".tsv")
results.posterior.inclusion.cor = paste0("results.posterior-inclusion-cov",SUFFIX,".tsv")
results.top.coefficients = paste0("results.top-coefficients",SUFFIX,".tsv")
...
# Locate the MCMC summary files: search for updated burning and thinning interval
summaryfiles = dir(".",glob2rx(paste0("doublethink-mcmc-summary-out.ID*",SUFFIX,".RDS")))
...
# Load one chain directly to obtain parameter details, removing burnin and thinning suffix
chainfiles = gsub(SUFFIX,"",gsub("summary-","",summaryfiles))
...
# Move current version of results.clusters.out
if(file.exists("results.clusters.out")) {
    i = 1
    while(file.exists(paste0("results.clusters.out.",i))) i = i+1
    system(paste0("mv results.clusters.out results.clusters.out.",i))
}
# Create factors for groups
system("bash -l", input=c("shopt -s expand_aliases", paste0(python_loc, " ", sourcedir, "/cluster_posterior_probabilities.py -cov ", results.posterior.inclusion.cor)))
...
write.table(out_means[2:length(out_means)], file=results.group.posteriors.inclusion.probs, quote=F,sep="\t", col.names=c("group_name\tcombined_PoP"))
#system("bash -l", input=c("shopt -s expand_aliases", "python -c 'import os; print(os.getcwd())'"))
system("bash -l", input=c("shopt -s expand_aliases", paste0(python_loc, " ", sourcedir, "/postpostprocessing-again.py -burnin ",burnin," -thinning ",thinning)))
