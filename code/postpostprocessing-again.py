"""
showThis script uses follows the postprocess-mcmc-again.R script that runs on the .RDS
...
containing all plots that are created. The script is run within the
results folder with the specified burnin and thinning
"""
import argparse
import pandas as pd
import numpy as np
...
def PoP_table(SUFFIX, config):
    ...
    # Read in table
    global number_subsample
    input_table, pheno_table, showcase_file, encoding_file, number_subsample = read_in_config()
    beta_df = pd.read_table("results.top-coefficients{}.tsv".format(SUFFIX))
    ...
    df = pd.read_table("results.posterior-inclusion-probs{}.tsv".format(SUFFIX))
    ...
    if number_subsample == 0:
        number_subsample = data_df.shape[0]
...
# TO DO: NEED TO ADD BURN-IN HERE
def get_prediction():
    """
...
def group_PoP(output_string, pop_df, pop_dict, beta_dict, coding_covariates, SUFFIX, config):
    """
    ...
    with open("results.group-posterior-inclusion-probs{}.tsv".format(SUFFIX)) as inf:
        for line in inf.readlines()[1:]:
			...
			# Split the group into individuals: delimiter defined postprocess-mcmc-again.R line 301
...
def pop_cov(output_string, pop_dict, coding_covariates, SUFFIX):
    """
    ...
    # Read in the table
    df = pd.read_table("results.posterior-inclusion-cov{}.tsv".format(SUFFIX))
...
def read_in_config():
    ...
    return(input_table, pheno_table, showcase_file, encoding_file, number_subsample)
...
def plot_posteriors(pdf, pop_dict, pop_df, beta_dict, data_df, SUFFIX):
    """
    ...
    plt.tight_layout()
    plt.savefig("results.top-posteriors-barplot{}.png".format(SUFFIX))
    pdf.savefig(fig)

    return(pdf)


def plot_contingency(pdf, data_df, pop_df, pop_dict, coding_covariates, SUFFIX):
    """
    ...
    ax.xaxis.tick_top()
    plt.tight_layout()
    plt.savefig("results.top-posteriors-contingencies-heatmap{}.png".format(SUFFIX))
...
def plot_group(pdf, group_pop_dict, pop_dict, coding_covariates, SUFFIX):
    """
    ...
    for group, group_pop in groups:
        ...
        # Individual members of the group: delimiter defined postprocess-mcmc-again.R line 301
    ...
    plt.tight_layout()
    plt.savefig("results.group-posteriors-barplot{}.png".format(SUFFIX))
    pdf.savefig(fig)

    return(pdf)


def plot_cov(pdf, cov_df, top_correlations, pop_dict, beta_dict, SUFFIX):
    """
    ...
    fig = plt.gcf()
    plt.savefig("results.posterior-inclusion-cov-clustermap{}.png".format(SUFFIX))
    pdf.savefig(fig)
...
def read_config(name):
    ...
    config['mu'] = 1.0/(config['MU']*config['nu'] - 1.0) # Prior odds of variable inclusion
    config['h'] = 1.0/config['f_prior']                  # Scale parameter for coefficients
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Post-process again")
    parser.add_argument(
        "-burnin", type=str, default="0",
        help="Burn-in")
    parser.add_argument(
        "-thinning", type=str, default="1",
        help="Thinning interval")
    args = parser.parse_args()
    SUFFIX = ".burnin" + args.burnin + ".thinning" + args.thinning
    ...
    pop_dict, pop_df, beta_dict, data_df, coding_covariates, pheno_table = PoP_table(SUFFIX, config)
    ...
    pdf = plot_posteriors(pdf, pop_dict, pop_df.head(10), beta_dict, data_df, SUFFIX)
    # Plot contingency table
    pdf = plot_contingency(
        pdf, data_df, pop_df.head(10),
        pop_dict, coding_covariates, SUFFIX)

    # Get groupwise posterior probabilities
    output_string, group_pop_dict = group_PoP(
        output_string, pop_df, pop_dict, beta_dict, coding_covariates, SUFFIX, config)

    # Plot the groupwise posterior probabilities
    pdf = plot_group(pdf, group_pop_dict, pop_dict, coding_covariates, SUFFIX)

    # Get covariances
    output_string, top_correlations, df_cov = pop_cov(
        output_string, pop_dict, coding_covariates, SUFFIX)

    # Plot covariances
    pdf = plot_cov(pdf, df_cov, top_correlations, pop_dict, beta_dict, SUFFIX)
