...
def get_matrix(corr):
    ...
    # Get the file created by run-mcmc.R that correlates every covariate with
    # each other
...
def get_farthest_neighbour(distance_matrix, covariate_seeds, auto_switch):
    ...
    # automated
    if args.m == 0 or distance_matrix.shape[1]/args.l < args.m:

        covariate_seeds = list(all_cols)
        random.shuffle(covariate_seeds)
    ...
    for i, start_seed in enumerate(covariate_seeds):
        ...
        if distance_matrix.empty:
            if len(cov_trace_FN) == num_cols:
                ...
                iteration {}\nNow randomly choosing""".format(i))
                distance_matrix = distance_matrix_copy.copy()
...
def get_start_groups_NA(distance_matrix):
    """
...
def get_start_groups(distance_matrix):
    ...
    Get the one single starting covariate for every chain. Then fill the rest with furthest neighbour
    """
    # If no chain number is specified we can guess the correct amount of chains needed
    auto_switch = False
    if args.m == 0:
        args.m = int(len(distance_matrix.columns) / args.l)
        auto_switch = True

    # distance_matrix is a pandas DataFrame with labels as index and columns
    # pyclustering requires a list of points or a distance matrix as a list of lists

    # The pyclustering KMedoids expects a distance matrix as a list of lists:
    dist_matrix = distance_matrix.values.tolist()

    # Choose initial medoids indices, for example pick first m+2 arbitrarily or randomly
    # Here: select the first (m+2) indices for initialization, or you can implement your heuristic
    initial_medoids = list(range(args.m + 2))

    # Create KMedoids instance with precomputed distance matrix
    ...
    # Map indices back to your distance_matrix labels (assuming order matches)
    medoids = np.array(distance_matrix.index)[medoid_indices]

    # Then get farthest neighbour groups (your existing function)
    groups = list(
        get_farthest_neighbour(
            distance_matrix,
            medoids,
...
if __name__ == "__main__":
    ...
    # Name of the correlation file created by run-mcmc.R
